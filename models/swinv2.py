...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, f"input feature has wrong size. current shape: {x.shape}"
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 device='cpu', pixel_space='rgb', **kwargs):
        super().__init__()
        ...
        if pixel_space.lower() == "rgb":
            ...
            img_size=img_size, patch_size=4, emb_size=embed_dim, use_subblock=True, norm_layer=norm_layer, device=device, # note that device is cpu and dtype is float32 by default
            )
    ...
    def forward_features(self, y, cbcr=None):
    ...
    def forward(self, y, cbcr=None):
        x = self.forward_features(y, cbcr)
        x = self.head(x)
        return x